...
async def run_with_telegram():
    ...
    # Identitiy will be fetched during connector.start()
    logger.info("[MAIN] Initializing Agent session...")

    try:
        agent = get_agent()
        
        # Ensure agent is initialized (DatabaseReader, etc.)
        logger.debug("[MAIN] Starting agent.initialize()...")
        if not await agent.initialize():
            logger.error("[MAIN] Failed to initialize agent components.")
            return
    except Exception as e:
        logger.error(f"[MAIN] Startup Error during Agent initialization: {e}")
        import traceback
    ...
    async def heartbeat():
        try:
            while True:
                ...
                logger.debug("[MAIN] Heartbeat: Agent process is still alive and healthy.")
    ...
    try:
        ...
        logger.error(f"[MAIN] Critical Agent failure: {e}")
        import traceback
        logger.error(traceback.format_exc())
    finally:
