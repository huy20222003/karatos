...
def should_escalate_chat(state: ChatState) -> Literal["plan", "post_generate"]:
    """Decide whether to escalate from CHAT to PLAN or move to post-generation."""
    if state.get("final_decision") == "PLAN" and state.get("escalation_level", 0) > 0:
        logger.info("[ROUTING] ⬆️ Escalation detected. Pivoting CHAT -> PLAN.")
        return "plan"
