...
class SharedModelProvider:
    ...
    def get_model(cls) -> BaseChatModel:
        if cls._instance is None:
            ...
            if provider == "ollama":
                ...
                logger.info(f"[MODEL_PROVIDER] Initializing DeepSeek: Model={settings.deepseek_model_name}")
                cls._instance = ChatDeepSeek(
                ...
                logger.info("[MODEL_PROVIDER] Initializing Claude Web shared adapter")
                cls._instance = ClaudeWebAdapter()
            else:
    ...
    async def warmup(cls):
        ...
        if provider == "ollama":
            ...
            warm_name = getattr(settings, "deepseek_model_name", "deepseek")
        elif provider == "claude_web":
        ...
        for attempt in range(max_retries + 1):
            try:
                ...
                if tools and hasattr(response, 'tool_calls') and response.tool_calls:
                    # Record Telemetry (est with tool names)
                    telemetry.record_interaction(telemetry.estimate_tokens(prompt + str(response.tool_calls)), latency)
                ...
                # Record Telemetry
                telemetry.record_interaction(telemetry.estimate_tokens(prompt + content), latency)
