...
def get_vision_model() -> Tuple[object, VisionModelConfig]:
    ...
    if provider == "claude-web":
        ...
        return ClaudeWebAdapter(), VisionModelConfig(provider="claude-web", model_name=model_name)

    raise ValueError(f"[VISION_PROVIDER] Provider '{provider}' does not support vision or is not configured.")
...
def build_vision_human_message(*, provider: str, prompt: str, image_base64: str, mime_type: str):
    ...
    if provider == "anthropic" or provider == "claude-web":
