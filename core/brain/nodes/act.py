...
async def chat_act_node(state: ChatState) -> ChatState:
    ...
    """
    logger.debug(f"[NEURAL_ACT] Starting action execution for {len(state.get('active_task', [])) if isinstance(state.get('active_task'), list) else 1} tasks...")
    # Support Parallel Execution (List of tasks)
    decisions = state.get("active_task")
