...
async def chat_generate_node(state: ChatState) -> ChatState:
    ...
    try:
        ...
        history_str = await ctx_manager.get_optimized_history(
            state["chat_id"], 
            state["context"]["memory"], 
            limit=settings.context_generation_limit,
        ...
        if prompt_mode == "fast_track":
            ...
            prompt = p_registry.get("persona.generator.pure_chat", current_time=current_time, logic=logic, history_str=history_str, msg=msg_for_prompt, peers=peers_list, associative_context=associative_context, user_pronoun=user_pronoun, bot_pronoun=bot_pronoun, bot_name=bot_name, mood=state.get('mood', 'OPTIMISTIC'), energy=f"{state.get('energy_level', 1.0)*100:.0f}%", language=lang_val)
        else:
            prompt = p_registry.get("persona.generator.synthesis", current_time=current_time, logic=logic, history_str=history_str, user_message=state.get("user_message", ""), msg=msg_for_prompt, peers=peers_list, thought=thought, results_str=results_str, associative_context=associative_context, user_pronoun=user_pronoun, bot_pronoun=bot_pronoun, bot_name=bot_name, mood=state.get('mood', 'OPTIMISTIC'), energy=f"{state.get('energy_level', 1.0)*100:.0f}%", language=lang_val)
        ...
        state["response"] = await enhance_response(
            state["response"],
