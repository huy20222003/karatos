...
class MemoryCategory(str, Enum):
    ...
    INTUITION = "INTUITION"       # Hunches, unspoken insights, internal hypotheses
    VAULT = "VAULT"               # Encrypted secrets, keys, sensitive data
...
class PersistentMemory:
    ...
    # ==========================================
    # SPECIALIZED MEMORY WRITERS (Phase 26)
    # ==========================================

    async def record_a2a_communication(self, target_agent: str, message: str, outcome: str = "PENDING"):
        """Record communication between this agent and another (Agent-to-Agent)."""
        key = f"a2a:{target_agent}:{datetime.utcnow().timestamp()}"
        await self.remember(
            key=key,
            value={"target": target_agent, "message": message, "outcome": outcome},
            category=MemoryCategory.A2A,
            importance=0.6
        )

    async def record_intuition(self, insight: str, confidence: float, context: Optional[str] = None):
        """Record an internal hunch or unspoken intuition."""
        key = f"intuition:{datetime.utcnow().timestamp()}"
        await self.remember(
            key=key,
            value={"insight": insight, "confidence": confidence, "context": context},
            category=MemoryCategory.INTUITION,
            importance=0.5
        )

    async def store_secret(self, secret_key: str, value: str, description: str = ""):
        """Store a sensitive secret in the vault (Markdown storage already uses write_encrypted)."""
        key = f"vault:{secret_key}"
        await self.remember(
            key=key,
            value={"key": secret_key, "value": value, "description": description},
            category=MemoryCategory.VAULT,
            importance=1.0
        )
    ...
    async def record_chat_message(self, chat_id: str, role: str, content: str, episode_id: Optional[str] = None):
        ...
        if episode_id:
            value["metadata"] = {"episode_id": episode_id}
            ...
            try:
                ...
                if user_msg:
                    ...
                    for unit in units:
                        ...
                        await self.remember(
                            key=f"distilled:{chat_id}:{cat_str.lower()}:{datetime.utcnow().timestamp()}",
                            value=unit.get("content"),
                            category=cat,
                            importance=unit.get("importance", 0.5)
                        )
                # Emotional states are now captured as distilled EMOTION units in memory/emotions/
                # High-level personality evolution still tracks interaction frequency
    ...
    async def get_chat_history(self, chat_id: str, limit: int = 15, episode_id: Optional[str] = None) -> list[dict]:
        ...
        try:
            ...
            # --- NGO FIX: If filtering by episode, we might need more than 'limit' raw entries 
            # to find enough matching messages.
            raw_limit = limit if not episode_id else limit * 2
            
            # OPTIMIZATION: Only load relevant tail from markdown file
            entries = self.md_storage.load_all_from_file(file_path, limit_last=raw_limit)
            
            if entries:
                # Sort by created_at
                entries.sort(key=lambda x: x.created_at)
                
                history = [e.value for e in entries]
                
                # Filter by Episode
                if episode_id:
                    history = [msg for msg in history if msg.get("metadata", {}).get("episode_id") == episode_id]
                
                history = history[-limit:]
                logger.debug(f"[MEMORY] Recovered {len(history)} messages for chat {chat_id} (Episode: {episode_id}).")
                return history
